...
import karlAsset from "@/assets/koehler-rauch.jpg.asset.json";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:image", content: fireAsset.url },
    ],
  }),
  component: Index,
});
    ...
    <PageFrame folio="I">
      ...
      <div id="prolog" className="journey-line">
        <RevealChapter index="I">
          ...
          <figure className="charred-photo"><img src={karlAsset.url} alt={t("Karl Josef Stoll am rauchenden Meiler", "Karl Josef Stoll beside the smoking kiln")} /><figcaption>Karl Josef Stoll · {t("Köhler aus Leidenschaft", "Charcoal maker by passion")}</figcaption></figure>
        </RevealChapter>
        <RevealChapter index="II">
          <figure className="charred-photo"><img src={fireAsset.url} alt={t("Brennender historischer Holzmeiler", "Burning traditional charcoal kiln")} /><figcaption>{t("Wenn der Meiler zu atmen beginnt", "When the kiln begins to breathe")}</figcaption></figure>
        ...
        <RevealChapter index="III">
          <figure className="charred-photo"><img src={coalAsset.url} loading="lazy" alt={t("Fertige Holzkohlestücke aus dem Meiler", "Finished charcoal pieces from the kiln")} /><figcaption>{t("Das schwarze Gold – bereit aus dem Meiler", "Black gold – ready from the kiln")}</figcaption></figure>
